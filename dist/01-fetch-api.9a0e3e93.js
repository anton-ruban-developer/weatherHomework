...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/01-fetch-api.js":[function(require,module,exports) {
...
function getWeatherData(city) {
  var apiKey = '5bfef379c91d8f9c765a003594d5da40';
...
function displayWeather(data) {
  ...
  cityCountry.textContent = "".concat(data.location.name, ", ").concat(data.location.country);
  localTime.textContent = data.location.localTime;
  temperature.textContent = "".concat(weather.temperature, "\xB0C");
}
weatherForm.addEventListener('submit', function (event) {
  event.preventDefault();
  var city = document.querySelector('#city').value;
  getWeatherData(city).then(function (weatherData) {
    displayWeather(weatherData);
  }).catch(function (error) {
    alert('Кажется что-то пошло не по плану', error);
  });
...
getWeatherData(defaultCity).then(function (weatherData) {
  displayWeather(weatherData);
}).catch(function (error) {
  console.error('Кажется что-то пошло не по плану', error);
});
